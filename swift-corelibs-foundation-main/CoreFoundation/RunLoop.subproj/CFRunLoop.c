...
static int32_t __CFRunLoopRun(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFTimeInterval seconds, Boolean stopAfterHandle, CFRunLoopModeRef previousMode) {
    ...
    do {
        ...
        // 如果处理了所有标记了信号的source0？
        if (sourceHandledThisLoop) {
            __CFRunLoopDoBlocks(rl, rlm);
        }
        // 如果处理了所有的source0？或者超时时间为0
	...
	// do not do any user callouts after this point (after notifying of sleeping)
        ...
        do {
            ...
            if (modeQueuePort != MACH_PORT_NULL && livePort == modeQueuePort) {
                ...
                // __CFArmNextTimerInMode 里面会调用dispatch_source_set_timer，时间到后会将_timerFired设为YES
                if (rlm->_timerFired) {
    ...
    if (modeName == kCFRunLoopCommonModes) {
        ...
        if (NULL != rlm && !CFSetContainsValue(rlm->_sources0, rls) && !CFSetContainsValue(rlm->_sources1, rls)) {
            if (0 == rls->_context.version0.version) {
                ...
                CFSetAddValue(rlm->_sources1, rls);//port
                __CFPort src_port = rls->_context.version1.getPort(rls->_context.version1.info);
                if (CFPORT_NULL != src_port) {
                    CFDictionarySetValue(rlm->_portToV1SourceMap, (const void *)(uintptr_t)src_port, rls);
                    __CFPortSetInsert(src_port, rlm->_portSet);// 有source1的port，rl的wakeport，timer的port
                }
            }
            __CFRunLoopSourceLock(rls);
            if (NULL == rls->_runLoops) {
